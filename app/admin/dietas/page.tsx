...
function DietasContent() {
  ...
  async function carregarDietaAtual() {
    ...
    setCarregando(false);
    if (!dieta) { alert('Este paciente não possui dieta ativa. Crie uma nova.'); return; }
    setDietaId(dieta.id);
    ...
    for (const ref of dieta.refeicoes) {
      refsMap.set(ref.tipo, {
        ...
        alimentos: ref.alimentos.map((al: { nome: string; quantidade: string; observacao?: string; receitaId?: string }) => ({
          nome: al.nome, quantidade: al.quantidade, observacao: al.observacao || '', receitaId: al.receitaId || '',
        })),
      });
    }
  ...
  function resetTudo() {
    setTitulo(''); setRefeicoes(criarRefeicaoVazia()); setDietaId(null); setModoEdicao(false); setSucesso(false);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (modoEdicao && dietaId) {
      await fetch('/api/admin/dietas', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ dietaId, titulo, refeicoes: refsPayload }) });
    } else {
      await fetch('/api/admin/dietas', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ clienteId, titulo, refeicoes: refsPayload }) });
    }
    setSalvando(false);
    setSucesso(true);
  }
  ...
  if (sucesso) {
    return (
      <>
        <PrintDieta titulo={titulo} paciente={clienteSelecionado?.nome} refeicoes={refeicoesFilled} variant="button" />
        <main className="max-w-lg mx-auto px-6 py-16 text-center">
          <div className="bg-white/80 backdrop-blur-sm rounded-3xl p-10 border border-cream-200 shadow-sm animate-fade-slide-in">
            <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6" style={{background:'#f0faf5'}}>
              <svg className="w-10 h-10" style={{color:'#1a8558'}} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex gap-3">
                <Link href="/admin" className="flex-1 text-center text-white py-3 rounded-xl font-medium transition-all hover:opacity-90" style={{backgroundColor:'#ff7a55'}}>
                  Painel
                </Link>
                <button onClick={resetTudo} className="flex-1 border border-cream-300 text-warm-600 py-3 rounded-xl font-medium hover:bg-cream-50 transition-all">
                  Nova Dieta
                </button>
              </div>
            </div>
    ...
    <main className="max-w-4xl mx-auto px-6 py-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-cream-200 shadow-sm">
          ...
          <div className="flex gap-3">
            <select id="select-cliente" value={clienteId}
              onChange={(e) => { setClienteId(e.target.value); setModoEdicao(false); setDietaId(null); }}
              className="flex-1 border border-cream-300 rounded-xl px-4 py-3.5 bg-cream-50 text-warm-800 focus-visible:ring-2 focus-visible:ring-sage-400 transition-all appearance-none" required>
              <option value="">Selecione um paciente...</option>
              {clientes.map(c => <option key={c.id} value={c.id}>{c.nome} — {c.email}</option>)}
            </select>
            <button type="button" onClick={carregarDietaAtual} disabled={!clienteId || carregando}
              ...
              {carregando ? '⏳ Carregando...' : '📋 Editar Atual'}
            </button>
          </div>
          {clienteSelecionado && (
            <div className="mt-2 flex items-center gap-2">
              <p className="text-sm font-medium" style={{color:'#1a8558'}}>✓ {clienteSelecionado.nome}</p>
              {modoEdicao && <span className="text-xs bg-amber-100 text-amber-700 px-2.5 py-0.5 rounded-lg font-medium">Modo edição</span>}
            </div>
          )}
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-cream-200 shadow-sm">
          ...
          <input id="dieta-titulo" type="text" placeholder="Ex: Plano Introdução Alimentar – 6 meses"
            value={titulo} onChange={(e) => setTitulo(e.target.value)}
            className="w-full border border-cream-300 rounded-xl px-4 py-3.5 text-lg bg-cream-50 text-warm-800 placeholder-warm-400 focus-visible:ring-2 focus-visible:ring-sage-400 transition-all" required />
        </div>
        ...
        <div className="flex gap-3 items-center justify-between text-sm bg-cream-100 rounded-xl px-4 py-3 border border-cream-200">
          <div className="flex items-center gap-2 text-warm-500">
            <span>📊</span>
            <span>{alimentosPreenchidos} alimentos em {refeicoesFilled.length} refeições</span>
          </div>
          {modoEdicao && <button type="button" onClick={resetTudo} className="text-xs text-warm-400 hover:text-danger underline">Descartar e criar nova</button>}
        </div>
          ...
          {refeicoes.map((ref, refIdx) => {
            ...
            return (
              <div key={ref.tipo} className="bg-white/80 backdrop-blur-sm rounded-2xl border border-cream-200 shadow-sm overflow-hidden">
                <button type="button" onClick={() => toggleRefeicao(refIdx)}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {temAlimentos && (
                      <span className="text-xs font-medium px-2 py-0.5 rounded-lg" style={{background:'#f0faf5', color:'#166947'}}>
                ...
                {ref.aberto && (
                  <div className="px-6 pb-5 pt-2 border-t border-cream-100 animate-fade-slide-in">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <input type="text" value={ref.horarioSugerido}
                      ...
                      {ref.alimentos.map((al, alIdx) => (
                        <div key={alIdx} className="bg-cream-50/50 rounded-xl p-3 border border-cream-100 group">
                          <div className="flex gap-2 items-center">
                            ...
                            <input placeholder="Alimento" value={al.nome}
                            ...
                            <input placeholder="Qtd" value={al.quantidade}
                            ...
                            <button type="button" onClick={() => removeAlimento(refIdx, alIdx)}
                              className="opacity-0 group-hover:opacity-100 min-w-[32px] min-h-[32px] flex items-center justify-center rounded-lg text-warm-300 hover:text-danger hover:bg-red-50 transition-all" aria-label="Remover">✕</button>
                          </div>
                          <div className="mt-2 ml-7 flex items-center gap-2">
                            <span className="text-xs text-warm-400">📖 Receita:</span>
                            <select value={al.receitaId}
                              ...
                              {receitas.map(r => <option key={r.id} value={r.id}>{r.titulo}{r.tempoPreparo ? ` (${r.tempoPreparo})` : ''}</option>)}
                            </select>
                            {al.receitaId && <span className="text-xs font-medium" style={{color:'#1a8558'}}>✓</span>}
                          </div>
                        </div>
                      ))}
                    </div>
                    <button type="button" onClick={() => addAlimento(refIdx)}
                      className="mt-3 inline-flex items-center gap-1.5 text-sm font-medium px-3 py-2 rounded-lg transition-all hover:bg-sage-50" style={{color:'#1a8558'}}>
        ...
        <button type="submit" disabled={salvando || !clienteId || !titulo}
          ...
          style={{background: modoEdicao ? '#d97706' : 'linear-gradient(135deg,#1a8558,#0f3d29)'}}>
          {salvando ? 'Salvando...' : modoEdicao ? '✏️ Salvar Alterações' : '✓ Criar Dieta'}
        </button>
      </form>
    </main>
  );
