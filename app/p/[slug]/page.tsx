...
export default async function LandingPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  } catch { /* banco offline — usa defaults */ }

  const servicosFinal = servicosDB.length > 0
    ? servicosDB.map((s: { titulo: string; descricao: string; icone?: string; imagemUrl?: string }) => ({ titulo: s.titulo, desc: s.descricao, icon: s.icone || '🥗', img: s.imagemUrl || '' }))
    : SERVICOS_DEFAULT;
  const planosFinal = planosDB.length > 0 ? planosDB : PLANOS_DEFAULT;
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6 py-20 grid md:grid-cols-2 gap-12 items-center">
          <div className="animate-fade-slide-up">
            ...
            <div className="flex flex-wrap gap-4">
              <Link href={loginHref} style={{backgroundColor:'#ff7a55'}} className="text-white px-7 py-4 rounded-2xl font-semibold shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all duration-300 min-h-[52px] flex items-center gap-2">
                Agendar Consulta
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {planosFinal.map((plano) => (
              <div key={plano.nome} className="relative rounded-3xl p-10 transition-all duration-300 hover:-translate-y-1" style={plano.destaque ? {background:'linear-gradient(135deg,#1a8558,#0f3d29)',boxShadow:'0 8px 40px rgba(26,133,88,0.3)',color:'white'} : {background:'#ffffff',boxShadow:'0 4px 24px rgba(0,0,0,0.07)',border:'1px solid #ede9e2'}}>
                ...
                <Link href={loginHref} className="block text-center w-full py-3.5 rounded-xl font-semibold transition-all duration-200 min-h-[48px]" style={plano.destaque ? {backgroundColor:'white', color:'#1a8558'} : {backgroundColor:'#1a8558', color:'white'}}>
                  Começar Agora
                </Link>
              </div>
            ))}
        ...
        <section className="py-24 px-6 bg-stone-100">
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {avaliacoes.map((a, i) => (
                <div key={i} className="bg-white rounded-3xl p-7" style={{boxShadow:'0 4px 20px rgba(0,0,0,0.07)'}}>
      ...
      <section className="py-24 px-6 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <Link href={loginHref} className="text-white px-8 py-4 rounded-2xl font-semibold shadow-lg hover:scale-[1.02] transition-all duration-300 min-h-[52px]" style={{backgroundColor:'#ff7a55'}}>
              Agendar Consulta
            </Link>
          ...
          <div className="grid md:grid-cols-12 gap-10 pb-12 border-b border-white/10">
            ...
            <div className="md:col-span-3">
              ...
              <ul className="space-y-3 text-sm">
                {[[`/${slug}#sobre`,'Sobre mim'],[`/${slug}#servicos`,'Serviços'],[`/${slug}#planos`,'Planos & Valores']].map(([href, label]) => (
                  <li key={href}>
                    <Link href={href} className="text-stone-400 hover:text-white transition-colors flex items-center gap-2 group">
                      <span className="w-1 h-1 rounded-full bg-stone-600 group-hover:bg-coral-400 transition-colors" />
                      {label}
                    </Link>
                  </li>
                ))}
            ...
            <div className="md:col-span-4">
              ...
              <ul className="space-y-4 text-sm">
                <li className="flex items-start gap-3 text-stone-300"><span className="text-base mt-0.5">📍</span><span>{endereco}</span></li>
                <li className="flex items-start gap-3 text-stone-300"><span className="text-base mt-0.5">📞</span><span>{telefone}</span></li>
                <li className="pt-2">
                  <Link href={loginHref} className="inline-flex items-center gap-2 text-white text-sm font-semibold px-5 py-2.5 rounded-xl transition-all duration-200 hover:opacity-90" style={{backgroundColor:'#ff7a55'}}>
                    Agendar Consulta
